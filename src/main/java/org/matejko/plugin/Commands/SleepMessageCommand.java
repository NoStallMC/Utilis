        ...
        if (commandName.equalsIgnoreCase("sleepmessage")) {
            ...
            if (args[0].equalsIgnoreCase("set")) {
                if (p.hasPermission("utilis.sm")) {
                    if (args.length < 2) {
                        p.sendMessage(ChatColor.RED + "You must provide a message to set.");
                        return false;
                    }
                ...
                if (!p.hasPermission("utilis.sm.admin")) {
                    p.sendMessage(ChatColor.RED + "You do not have permission to edit other players' messages.");
                    return false;
                }
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "You must provide a player name.");
                    return false;
                }
                String playerName = args[1];
                String message = messages.getCustomSleepMessage(playerName);
                if (message == null || message.isEmpty()) {
                    p.sendMessage(ChatColor.RED + "No custom message found for " + playerName);
                } else {
                    editingPlayerMap.put(p, playerName); // Store the player being edited
                    p.sendMessage(ChatColor.GREEN + playerName + "'s current custom message: " + ChatColor.WHITE + message);
                    p.sendMessage(ChatColor.YELLOW + "Use /sleepmessage new <new message> to set a new message for " + playerName);
                }
                return true;
            } else if (args[0].equalsIgnoreCase("new")) {
                if (!editingPlayerMap.containsKey(p)) {
                    p.sendMessage(ChatColor.RED + "You are not editing any player's message. Use /sleepmessage edit <playername> first.");
                    return false;
                }
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "You must provide a new message.");
                    return false;
                }
                ...
                p.sendMessage(ChatColor.GREEN + "Successfully changed " + playerName + "'s custom sleep message to:  " + ChatColor.WHITE + newMessage);
                return true;
            } else if (args[0].equalsIgnoreCase("remove")) {
                if (!p.hasPermission("utilis.sm.admin")) {
                    p.sendMessage(ChatColor.RED + "You do not have permission to remove other players' messages.");
                    return false;
                }
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "You must provide a player name.");
                ...
                if (messages.removeCustomSleepMessage(playerName)) {
                    p.sendMessage(ChatColor.GREEN + playerName + "'s custom message has been removed.");
                } else {
                    p.sendMessage(ChatColor.RED + "No custom message found for " + playerName);
                }
                return true;
            } else {
                p.sendMessage(ChatColor.RED + "Invalid command syntax!");
                return false;
            }
        }
        return true;
