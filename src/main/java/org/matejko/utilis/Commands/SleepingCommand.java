    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (!player.hasPermission("utilis.ns")) {
                player.sendMessage("§7[§2Utilis§7] " + ChatColor.RED + "You do not have permission to use this command.");
                return true;
            }
    ...
    private boolean handlePlayerCommand(Player player, String[] args) {
        if (args.length == 0) {
            ...
            if (sleepingManager.isSleepingEnabled(player.getWorld())) {
                player.sendMessage("§7[§2Utilis§7] " + "Sleeping is currently enabled in this world!");
            } else {
                player.sendMessage("§7[§2Utilis§7] " + "Sleeping is currently disabled in this world!");
            }
            ...
            if (sleepingManager.isSleepingEnabled(player.getWorld())) {
                player.sendMessage("§7[§2Utilis§7] " + "Sleeping is now enabled in this world!");
            } else {
                player.sendMessage("§7[§2Utilis§7] " + "Sleeping is now disabled in this world!");
            }
        } else {
            player.sendMessage("§7[§2Utilis§7] " + ChatColor.RED + "Invalid command syntax! Use '/ns' to check the sleeping status or '/ns toggle' to toggle sleeping.");
        }
        return true;
    }
        ...
        if (args.length == 0) {
            ...
            for (World world : Bukkit.getWorlds()) {
                if (sleepingManager.isSleepingEnabled(world)) {
                    sender.sendMessage("§7[§2Utilis§7] " + "Sleeping is enabled in world '" + world.getName() + "'.");
                } else {
                    sender.sendMessage("§7[§2Utilis§7] " + "Sleeping is disabled in world '" + world.getName() + "'.");
                }
            }
            ...
            if (world != null) {
                if (sleepingManager.isSleepingEnabled(world)) {
                    sender.sendMessage("§7[§2Utilis§7] " + "Sleeping is currently enabled in world '" + worldName + "'.");
                } else {
                    sender.sendMessage("§7[§2Utilis§7] " + "Sleeping is currently disabled in world '" + worldName + "'.");
                }
            } else {
                sender.sendMessage("§7[§2Utilis§7] " + ChatColor.RED + "World not found: " + worldName);
            }
            ...
            sender.sendMessage("§7[§2Utilis§7] " + ChatColor.RED + "Invalid command! Use '/ns' to check all worlds, '/ns worldname' to check a specific world, or '/ns toggle worldname' to toggle sleeping.");
        }
        return true;
    }
}
